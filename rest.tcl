...
proc ::rest::RecConfigCopy {call {Level 1}} {
	upvar $Level in inTmp
	
	# puts "Debug Call --> $call"
	
	set config $inTmp($call)
	
	if {[dict exists $config copy]} {
		incr Level
		
		foreach copyName [dict get $config copy] {

			set config_parent [[lindex [info level 0] 0] $copyName $Level]
			set config [dict merge $config_parent $config]
			
			if {[dict exists $config child_url] && [dict exists $config url]} {
				dict set config url [join [list [dict get $config url] [dict get $config child_url]] /]
				dict unset config child_url
			}
		}
	}
	return $config
...
proc ::rest::create_interface {name} {
    ...
    namespace eval ::$name {}
    #create interface for every name except for names enclosed with @. Names enclosed with @ are used as templates
    foreach call [array names in -regexp {^[^@]}] {
        ...
        if {[dict exists $config headers]} {
 		...
 		#add static_args, opt_args, req_args
		set AddConfigDict [dict filter $config key +*]
		if [dict size $AddConfigDict] {
			dict for {Key Value} $AddConfigDict {
				switch -- $Key {
					+static_args -
					+opt_args -
					+req_args {
        ...
        if {[dict exists $config auth]} {
			...
			switch -- $auth {
				...
				command {
					lappend proc [subst -nocommands {
						lappend headers Authorization [::${name}::Command_auth]
					}]
					
					if {[info commands ::${name}::command_auth] == ""} {
						proc ::${name}::Command_auth {} {
							variable command_auth_cmd
							return [try $command_auth_cmd]
						}
						
						proc ::${name}::command_auth {ScrCmd} {
							variable command_auth_cmd $ScrCmd
						}						
					}					
				}
			}
...
# CLIF FIX
        ...
        if {[dict exists $config callback]} {
			...
			set proc [linsert $proc 0 [subst -novariables -nocommands {
				if [dict exists $args -callback] {
					set callback_cmd [dict get $args -callback]
					dict unset args -callback
				}
			}]]	
		
			lappend proc [subst -nocommands {if ![info exists callback_cmd] \{}]
			...
			lappend proc [subst -nocommands {set t [::rest::_call [list ::${name}::_callback_$call \$callback_cmd] \$headers \$url \$query \$body \$error_body]}]
			lappend proc {return $t}
...
#       nothing
#
proc ::rest::_callback {error_body datacb usercb t} {
    ...
    if {![string match 2* $httpCode]} {
		set retList [list]
        if {[string match {30[123]} $httpCode]} {
            lappend retList [dict get $token(meta) Location]
        }
		if {$error_body} {
			lappend retList [http::data $t]
		}
		
		http::cleanup $t
		
		set RetHttpCode [list HTTP $httpCode]
		{*}$usercb ERROR $RetHttpCode $retList
        return
    }
